...
<form id="salary_structure_edit_form" enctype="multipart/form-data" method="post" data-toggle="validator" novalidate="true">
    ...
    <div class="row">
        ...
        <div class="col-sm-11 offset-sm-1">
            ...
            <!-- new start -->
            <div class="row">
                <input type="text" value="" name="pf_id" id="edit_pf_id_alt" hidden>
                <input type="text" value="" name="wf_id" id="edit_wf_id_alt" hidden>
                <input type="text" value="" name="ps_id" id="edit_ps_id_alt" hidden>
                <input type="text" value="" name="osf_id" id="edit_osf_id_alt" hidden>
                <input type="text" value="" name="inc_id" id="edit_inc_id_alt" hidden>

                <div id="edit_pf_div" class="col-sm-5 form-group d-none">
                    <label class="input-title ">Select PF</label>
                    <div class="input-group">
                        <select name="pf_id" id="edit_pf_id" style="width: 100%;" class="form-control clsSelect2"></select>
                    </div>
                </div>
                

                <div id="edit_wf_div" class="col-sm-5  offset-sm-1 form-group d-none">
                    <label class="input-title ">Select WF</label>
                    <div class="input-group">
                        <select name="wf_id" id="edit_wf_id" style="width: 100%;" class="form-control clsSelect2"></select>
                    </div>
                </div>
                

                <div id="edit_ps_div" class="col-sm-5 form-group d-none">
                    <label class="input-title ">Select PS</label>
                    <div class="input-group">
                        <select name="ps_id" id="edit_ps_id" style="width: 100%;" class="form-control clsSelect2"></select>
                    </div>
                </div>
               

                <div id="edit_osf_div" class="col-sm-5  offset-sm-1 form-group d-none">
                    <label class="input-title ">Select OSF</label>
                    <div class="input-group">
                        <select name="osf_id" id="edit_osf_id" style="width: 100%;" class="form-control clsSelect2"></select>
                    </div>
                </div>

                <div id="edit_inc_two_div" class="col-sm-5 form-group d-none">
                    <label class="input-title ">Select Insurance</label>
                    <div class="input-group">
                        <select name="inc_id" id="edit_inc_id" style="width: 100%;" class="form-control clsSelect2"></select>
                    </div>
                </div>
...
<script>
    ...
    function isSalarayStructure(){
        $('#edit_project_id').change(function(event) {
            event.preventDefault();
            callApi("{{ url()->current() }}/../../getData", 'post', new FormData($('#salary_structure_edit_form')[0]),
                function(response, textStatus, xhr) {
                    // console.log(response);
                    // Start For PF
                    if (response.pfData.length > 1) {
                        $("#edit_pf_id_alt").attr("disable", true);
                        let selectPfOption = "<option value='' >Select</option>";
                        ...
                        $('#edit_pf_id').html(selectPfOption);
                        $("#edit_pf_div").removeClass('d-none');
                        $("#edit_pf_id").val("{{$editData->pf_id}}").trigger("change");
                    } else {
                        $("#edit_pf_id_alt").attr("disable", false);
                        $("#edit_pf_id_alt").val(response.pfData.id);
                        $("#edit_pf_div").addClass('d-none');
                    }
                    // End For PF
                    ...
                    if (response.epsData.length > 1) {
                        $("#edit_ps_id_alt").attr("disable", true);
                        let selectPsOption = "<option value='' >Select</option>";
                        $.each(response.epsData, function(index, item) {
                            selectPsOption += "<option value='"+ item.id +"'>"+ item.amount+" ( "+ item.effective_date +") </option>";
                        });
                        $('#edit_ps_id').html(selectPsOption);
                        $("#edit_ps_div").removeClass('d-none');
                        $("#edit_ps_id").val("{{$editData->ps_id}}").trigger("change");
                    } else {
                        $("#edit_ps_id_alt").attr("disable", false);
                        $("#edit_ps_id_alt").val(response.epsData.id);
                        $("#edit_ps_div").addClass('d-none');
                    }
                    // End For PS
                    ...
                    if (response.osfData.length > 1) {
                        $("#edit_osf_id_alt").attr("disable", true);
                        ...
                        $('#edit_osf_id').html(selectOsfOption);
                        $("#edit_osf_div").removeClass('d-none');
                        $("#edit_osf_id").val("{{$editData->osf_id}}").trigger("change");
                    } else {
                        $("#edit_osf_id_alt").attr("disable", false);
                        $("#edit_osf_id_alt").val(response.osfData.id);
                        $("#edit_osf_div").addClass('d-none');
                    }
                    // End For OSF

                    // Start For Insurance
                    if (response.incData.length > 1) {
                        $("#edit_inc_id_alt").attr("disable", true);
                        ...
                        $('#edit_inc_id').html(selectIncOption);
                        $("#edit_inc_two_div").removeClass('d-none');
                        $("#edit_inc_id").val("{{$editData->inc_id}}").trigger("change");
                    } else {
                        $("#edit_inc_id_alt").attr("disable", false);
                        $("#edit_inc_id_alt").val(response.incData.id);
                        $("#edit_inc_two_div").addClass('d-none');
                    }
                    // End For Insurance
    ...
    if ( $("#edit_grade_id").val() != '') {
        greadLevel();
        $('#edit_grade_id').trigger("change");
    }
    function greadLevel() {
        $("#edit_grade_id, #edit_level_id").change(function(event){
            event.preventDefault();

            callApi("{{ url()->current() }}/../../getWfData", 'post', new FormData($('#salary_structure_edit_form')[0]),
                function(response, textStatus, xhr) {
                    // Start For WF
                    ...
                    $('#edit_wf_id').html(selectWfOption);
                    $("#edit_wf_div").removeClass('d-none');
                    $("#edit_wf_id").val("{{$editData->wf_id}}").trigger("change");
                    // End For WF
