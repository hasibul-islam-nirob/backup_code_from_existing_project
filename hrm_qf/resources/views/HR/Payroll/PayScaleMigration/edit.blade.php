...
<form id="payroll_pay_scale_migration_edit_form" enctype="multipart/form-data" method="post" data-toggle="validator" novalidate="true">
    @csrf
    ...
    <div class="row">
        ...
        <div class="col-sm-5">
            <div id="employee_add_div" class="form-group">
                ...
                <div class="input-group">
                    <select id="edit_employee_id" name="employee_id" class="form-control clsSelect2" style="width: 100%" disabled>
        ...
        <input type="text" value name="branch_id" id="edit_branch_id_active" hidden>
        <input type="text" value name="employee_id" id="edit_employee_id_ective" hidden>
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="edit_be_grade" id="edit_be_grade" style="width: 100%" disabled>
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="be_level" id="edit_be_level" style="width: 100%" disabled>
                                <option value="">Select</option>
                                @for($i = 1; $i<= 4; $i++)
                                    <option value="{{ $i }}">{{ $i }}</option>
                                @endfor
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="be_org_payscal_year_id" id="edit_be_payscal_year_id" style="width: 100%" disabled>
                                <option value="">Select </option>
                                @foreach($payscaleData as $row)
                                    <option value="{{ $row->id }}">{{ $row->name }}</option>
                                @endforeach
                            </select>
                            <input type="text" value name="be_payscal_year_id" id="edit_be_payscal_year_id_org" hidden>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="be_step" id="edit_be_step" style="width: 100%" disabled>
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="af_grade" id="edit_af_grade" style="width: 100%">
                                <option value="">Select</option>
                                @for($i = 1; $i<= 7; $i++)
                                    <option value="{{ $i }}">{{ $i }}</option>
                                @endfor
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="af_level" id="edit_af_level" style="width: 100%">
                                <option value="">Select</option>
                                @for($i = 1; $i<= 4; $i++)
                                    <option value="{{ $i }}">{{ $i }}</option>
                                @endfor
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="af_org_payscale_year_id" id="edit_af_payscale_year_id" style="width: 100%">
                                <option value="">Select </option>
                                @foreach($payscaleData as $row)
                                    <option value="{{ $row->id }}">{{ $row->name }}</option>
                                @endforeach
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select class="form-control clsSelect2" name="af_org_step" id="edit_af_step" style="width: 100%">
                                <option value="">Select Step</option>
                                @for($i = 1; $i<=12; $i++)
                                    <option value="{{ $i }}">{{ $i }}</option>
                                @endfor
                ...
                <input type="text" value name="salaray_structure_id" id="edit_salaray_structure_id" hidden>
...
<script>
    ...
    $(document).ready(function(){
        let getBranchId = "{{$editData->branch_id}}";
        $("#edit_branch_id").val(getBranchId).trigger("change");
        $("#edit_branch_id").attr('disabled', true);
        $("#edit_branch_id_active").val(getBranchId);

        callApi("{{ url()->current() }}/../../get/" + "{{ $editData->id }}/api", 'post', '',

            function(response, textStatus, xhr) {

                console.log(response);
                $("#edit_employee_id").val(response.empID).trigger("change");
                $("#edit_employee_id_ective").val(response.empID);
                $("#edit_id").val(response.id);
                $("#rectuitment_type_id").val(response.af_recruitmentId);

                $("#edit_be_grade").val(response.be_grade).trigger("change");
                $("#edit_be_level").val(response.be_level).trigger("change");
                $("#edit_be_payscal_year_id").val(response.be_payscal_id).trigger("change");
                $("#edit_be_payscal_year_id_org").val(response.be_payscal_id).trigger("change");
                $("#edit_be_step").val(response.be_steps).trigger("change");

                $("#edit_af_grade").val(response.af_grade).trigger("change");
                $("#edit_af_level").val(response.af_level).trigger("change");
                $("#edit_af_payscale_year_id").val(response.af_payscal_id).trigger("change");
                $("#edit_af_step").val(response.af_steps).trigger("change");
    ...
    showModal({
        titleContent: "Edit Payroll Pay Scale Migration",
    ...
    $('#add_saveBtn').click(function(event) {
        ...
        callApi("{{ url()->current() }}/../../update/api", 'post', new FormData($('#payroll_pay_scale_migration_edit_form')[
                0]),
            function(response, textStatus, xhr) {
    ...
    $("#edit_af_step").change(function(){
        let empId = $("#edit_employee_id").val();
        let af_grade = $("#edit_af_grade").val();
        let af_level = $("#edit_af_level").val();
        let af_step = $("#edit_af_step").val();
        let af_payscale = $("#edit_af_payscale_year_id").val();
        setZeroAf()
        $.ajax({
            type: "POST",
            ...
            success: function (response) {
                ...
                if (response.After_SS_Data && response.After_SS_Data != null) {
                    ...
                    $("#edit_salaray_structure_id").val(response.After_SS_Data.salary_structure_id);
