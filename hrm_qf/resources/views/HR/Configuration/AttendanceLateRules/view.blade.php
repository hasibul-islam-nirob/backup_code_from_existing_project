            ...
            <div class="row form-group">
                ...
                <div class="col-sm-7 offset-sm-1 ">
                    <div class="input-group-prepend ">
                        ...
                        <input type="text" id="eff_date_start" disabled name="eff_date_start"
                            class="form-control datepicker-custom" placeholder="DD-MM-YYYY" autocomplete="off">
                    </div>
...
<script>
    ...
    $(".createLpRowIcon").on('click', function(event){
        ...
        $("#LpBreakdownTable").append(
            '<tr>'+
                ...
                '<td> '+text1+' <input id="lpDay'+trCount+'" type="text" disabled name="lateBreakdownName[]" value="'+dayInput+'" readonly style="border:0px; float: right; width: 5rem; margin-right: 1rem; text-align: center;" ></td>'+
                '<td><input id="ld'+trCount+'" type="text" disabled name="lateBreakdownNum[]" value="'+ldInput+'" readonly style="border:0px; float: right; width: 5rem; margin-right: 1rem; text-align: center;"></td>'+
                '<td class="" style="text-align: center;">'+
                    '<a disabled class="createLpRowIcon'+trCount+'"><i class="fa fa-minus-circle" style="color: red;"></i></a>'+
                '</td>'+
            '</tr>'
            )
    ...
    $(".AddActionForLpBtn").on('click', function(event) {
        ...
        $("#ActionForLpTable").append(
            '<tr>'+
                ...
                '<td class="" style="text-align: center;">'+
                    '<a class="createLpRowIcon'+trCount+'"><i class="fa fa-minus-circle" style="color: red;"></i></a>'+
                '</td>'+
            '</tr>'
            )
    ...
    $(document).ready(function(){
        ...
        $("#designation_id").attr('disabled', true)
        window.attData = [];
        window.flag = 0;
        ...
        callApi("{{ url()->current() }}/../../get/{{ $id }}/api", 'post', new FormData($('#attendance_late_rules_edit_form')[0]),
            function(response, textStatus, xhr) {
                ...
                // $("#setJsonStringLPB").html(response.lp_breakdown)
                ...
                if (1) {
                    ...
                    $.each(breakdownArray, function(index, obj) {
                        ...
                        '<tr>'+
                            '<td style="text-align: center;"> '+trCount+' </td>'+
                            '<td> '+text1+' <input id="lpDay'+trCount+'" type="text" disabled name="lateBreakdownName[]" value="'+breakdownKey+'" readonly style="border:0px; float: right; width: 5rem; margin-right: 1rem; text-align: center;" ></td>'+
                            '<td><input id="ld'+trCount+'" type="text" disabled name="lateBreakdownNum[]" value="'+breakdownValue+'" readonly style="border:0px; float: right; width: 5rem; margin-right: 1rem; text-align: center;"></td>'+
                            '<td class="" style="text-align: center;">'+
                                '<a class="createLpRowIcon'+trCount+'"><i class="fa fa-minus-circle" style="color: #797986;"></i></a>'+
                            '</td>'+
                        '</tr>'
                        )
                ...
                if (1) {
                    ...
                    $.each(deductionArray, function(index, obj) {
                        ...
                        $("#ActionForLpTable").append(
                            '<tr>'+
                                '<td style="text-align: center;"> '+trCount+' </td>'+
                                '<td> '+text1+' <input id="lpInput'+trCount+'" type="text" disabled name="lateDeductionName[]" value="'+deductionKey+'" readonly style="border:0px; float: right; width: 8rem; margin-right: 3rem; height: 20px; "></td>'+
                                '<td><input id="lpNum'+trCount+'" type="text" disabled name="lateDeductionNum[]" value="'+deductionValue+'" readonly style="border:0px; float: right; width: 5rem; margin-right: 1rem;  text-align: center;"></td>'+
                                '<td class="" style="text-align: center;">'+
                                    '<a disabled class="createLpRowIcon'+trCount+'"><i class="fa fa-minus-circle" style="color: #797986;"></i></a>'+
                                '</td>'+
                            '</tr>'
                        )
    ...
    showModal({
        titleContent: "View Attendance Late Rules",
    });
