...
<div id="draftView" style="display: none;">
    @include('HR.CommonBlade.detailsGrid',['title' => 'Demotion Application Details'])
</div>
...
<script>
        ...
        function(response, textStatus, xhr) {
            ...
            if(response.result_data['is_active'] == 0 || response.result_data['is_active'] == 1){//Draft view
                ...
                showApplicantData(response.result_data).then(()=>{
                    ...
                    showModal({
                        titleContent: "View Demotion Application",
                    });
    ...
    async function showApplicantData(response){
        ...
        html += '<tr>' +
            '<td style="width: 20%"><b>Demotion Code</b></td>' +
            '<td style="width: 27%">'+ response.demotion_code +'</td>' +
        ...
        html += '<tr>' +
            ...
            '<td style="width: 20%"><b>Demotion Date</b></td>' +
            '<td style="width: 30%">'+ response.demotion_date +'</td>' +
        '</tr>';
        ...
        html += '<tr>' +
            ...
            '<td style="width: 27%">'+ response.designation_to_demote.name +'</td>' +
    ...
    async function showApplication(response){
        let resData = new Date(response.demotion_date);
        ...
        $('#application_subject').html("Subject: Application for Demotion");
