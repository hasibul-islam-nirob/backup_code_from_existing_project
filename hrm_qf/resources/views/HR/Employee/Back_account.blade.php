                ...
                <div class="col-lg-6 form-group">
                    <div class="row">
                        ...
                        <div class="input-group col-lg-8">
                            ...
                            <input type='text' class="form-control" onfocus="branchOnChange(this)" id="acc_branch__1" name="acc_bank_branch_id[]" style="width: 100%">
                        </div>
                    </div>
...
<script>
    ...
    $(document).ready(function (){
        ...
        if(empAccData !== null && empAccData.length > 0){
            ...
            for (let i=0; i<empAccData.length; i++){
                ...
                // $.when(loadSelectBox({'bankId' : document.querySelectorAll("[name='acc_bank_id[]']")[i].value},'getBankBranches',document.querySelectorAll("[name='acc_bank_branch_id[]']")[i]))
                //     .then(function (){
                //         setEditData(document.querySelectorAll("[name='acc_bank_branch_id[]']")[i], empAccData[i]['bank_branch_id']);
                //     });
    ...
    function branchOnChange(inputElement){
        let nearestDiv = $(inputElement).closest('.bankBranchArea');
        let nearestSelect = nearestDiv.find('select.empBank');

        let bankId = nearestSelect.attr("id");
        let bankValue = nearestSelect.val();

        // let branchId = $(inputElement).attr("id");
        // let branchValue = $(inputElement).val();
        ...
        autocomplete(document.getElementById(branchId), bankBranchArray);
