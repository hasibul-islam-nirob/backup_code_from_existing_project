...
<form enctype="multipart/form-data" method="post" class="form-horizontal" 
            ...
            <div class="form-row form-group align-items-center">
                ...
                <div class="col-lg-5">
                    <div class="input-group">
                        ...
                        data-error="Title name  required" value="{{ $module_name }}" readonly
                        placeholder="Enter Title">

                        @foreach ($module_data as $module)
                            <input type = "hidden" name="module_id[]" value="{{ $module[0]->id}}" readonly> 
                        @endforeach
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
                ...
                <div class="col-lg-12" id="headDivID">
                    ...
                    <div class="row new-row" id="parentrow">
                        ...
                        <div class="col-lg-2 form-group">
                            ...
                            <div class="input-group">
                                <select class="form-control clsSelect2" onchange="applicationvaluemove(this.value, 0)" 
                                        data-error="Please Select Branch" id="select_2_1_main">
                        ...
                        <div class="col-lg-2 form-group">
                            ...
                            <div class="input-group">
                                <select class="form-control clsSelect2"
                                        data-error="Please Select Designation" id="select_2_2_main"
                                    ...
                                    @foreach($EmpDesignations as $key =>$des)
                                    
                                        <option value="{{ $des[$key]->id }}">{{ $des[$key]->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-lg-2 form-group">
                            ...
                            <div class="input-group">
                                <select class="form-control clsSelect2 emp" id="select_2_3_main"
            ...
            <div class="row">
                <input type="hidden" id="headtotalRow" value="{{ count($headData) }}">
                <div class="col-lg-12" id="headDivID">
                    @foreach ($headData as $index => $data)
                    <div class="row new-row" id="parentrow">
                        <div class="col-lg-2 form-group">
                            <label class="input-title RequiredStar">Title Name</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="head_title[]"
                                       data-error="Title name required" id="head_title_{{ $index }}" value="{{ $data->title }}"
                                       placeholder="Enter Title">
                            </div>
                            <div class="help-block with-errors is-invalid"></div>
                        </div>
                        <div class="col-lg-2 form-group">
                            <label class="input-title RequiredStar">Applicable For</label>
                            <div class="input-group">
                                <select class="form-control clsSelect2" onchange="applicationvaluemove(this.value, {{ $index }})"
                                        data-error="Please Select Branch" id="select_2_1_{{ $index }}">
                                    <option value="">Select</option>
                                    <option value="-1" {{ $data->applicableFor == -1 ? 'selected' : '' }}>All</option>
                                    <option value="1" {{ $data->applicableFor == 1 ? 'selected' : '' }}>Head Office</option>
                                    <option value="2" {{ $data->applicableFor == 2 ? 'selected' : '' }}>Branch Office</option>
                                </select>
                            </div>
                            <input type="hidden" name="head_signatorApplicable[]" id="application_{{ $index }}" value="{{ $data->applicableFor }}">
                            <div class="help-block with-errors is-invalid"></div>
                        </div>
                        <div class="col-lg-2 form-group">
                            <label class="input-title RequiredStar">Select Designation</label>
                            <div class="input-group">
                                <select class="form-control clsSelect2"
                                        data-error="Please Select Designation" id="select_2_2_{{ $index }}"
                                        onchange="fnChangeEmployeeHead({{ $index }}); loaddesignation(this.value, {{ $index }})"
                                    >
                                    <option value="">Select</option>
                                    <option value="-2" {{ $data->signatorDesignationId == -2 ? 'selected' : '' }}>N/A</option>
                                    <option value="-1" {{ $data->signatorDesignationId == -1 ? 'selected' : '' }}>Logged In User</option>
                                    @foreach($EmpDesignations as $key =>$des)
                                            <option value="{{ $des[$key]->id }}">{{ $des[$key]->name }}</option>
                                    @endforeach
                            ...
                            <input type="hidden" name="head_signatorDesignationId[]" id="designation_{{ $index }}" value="{{ $data->signatorDesignationId }}">
                            <div class="help-block with-errors is-invalid"></div>
                        </div>
                        <div class="col-lg-2 form-group">
                            <label class="input-title">Select Employee</label>
                            <div class="input-group">
                                <select class="form-control clsSelect2 emp" id="select_2_3_{{ $index }}"
                                        data-error="Please Select Employee" onchange="loademployee(this.value, {{ $index }})">
                                    <option value="">Select</option>
                                </select>
                            </div>
                            <input type="hidden" name="head_signatorEmployeeId[]" id="employee_{{ $index }}" value="">
                            <div class="help-block with-errors is-invalid"></div>
                        </div>
                        <div class="col-lg-2 form-group">
                            <label class="input-title RequiredStar">Order </label>
                            <div class="input-group">
                                <input type="text" class="form-control textNumber" name="head_positionOrder[]"
                                        data-error="Order is required" id="order_id_{{ $index }}" value="{{ $data->positionOrder }}"
                                    placeholder="Enter Order">
                            </div>
                            <div class="help-block with-errors is-invalid"></div>
                        </div>
...
$('.addrow').on('click', function() {
    ...
    if (isPassportCheckValid){
        ...
        var applicableFor = $('#select_2_1_main').val();
        var designation = $('#select_2_2_main').val();
        var employee = $('#select_2_3_main').val();
                                ...
                                <div class="col-lg-2 form-group">
                                    ...
                                    <div class="input-group">
                                        <select class="form-control clsSelect2" 
                                            ...
                                            @foreach($EmpDesignations as $key => $des)
                                                <option value="{{ $des[$key]->id }}" ${designation == "{{$des[$key]->id}}" ? 'selected' : ''}>{{ $des[$key]->name }}</option>
                                            @endforeach
