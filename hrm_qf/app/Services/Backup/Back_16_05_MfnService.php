...
class Back_16_05_MfnService
{
    ...
    public static function getLoanStatusTTL($loanIdOrIds, $byCurrentFieldOfficer, ...$dates)
    {
        $loanStatuses = array();
        ...
        if ("loan_collection_query") {
            ...
            $loanCollections = DB::table('mfn_loan_collections')
                ...
                ->selectRaw('loanId, productId, collectionDate, paymentType, amount, principalAmount, interestAmount');
    ...
    public static function generateLoanScheduleTTL($loanIdOrIds, ...$dates)
    {
        ...
        foreach ($loans as $key => $loan) {
            ...
            ### Query Optimization 22/03/23
            $schedule = self::getInstallmentDatesFromTable(($loan->id));
            $insallmentdates = json_decode($schedule[$loan->id]);
    ...
    public static function getInstallmentDatesFromTable($loanId)
    {
        
        $schedule = DB::table('mfn_loan_schedule')->where('loanId', $loanId)->pluck('schedule','loanId')->toArray();
        // dd($schedule);
    ...
    public static function generateLoanScheduleForTable($loanIdOrIds, ...$dates)
    {
        self::resetProperties();
        ...
        $schedules = self::generateInstallmentDatesMultiple($loanIdOrIds);
    ...
    public static function generateWeeklyInstallmentDates($loan)
    {
        ...
        if(empty(self::$massLoanReschedules)){
            self::$massLoanReschedules = DB::table('mfn_loan_mass_reschedules')->where('is_delete', 0)->get();
        }
    ...
    public static function generateMonthlyInstallmentDates_backup_23102022($loan)
    {
        $installmentDate = $loan->firstRepayDate;
    ...
    public static function generateMonthlyInstallmentDates($loan)
    {
        ...
        for ($i = 0; $i < $loan->numberOfInstallment && $willContinue; $i++) {
            ...
            // dd(self::$samity);
            // dd($installmentDate, $monthStartDate, $monthEndDate, self::$holidays);
    ...
    { ## updated function for fnForSavingsProductData()
        ...
        $queryData = DB::table('mfn_savings_product')
            ...
            ->get(['id', 'name', 'shortName', 'productCode', 'nameCode'])
            ->keyBy('id')
            ->toArray();
        return $queryData;
    }
