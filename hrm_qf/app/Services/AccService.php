...
class AccService
{
    ...
    public static function getLedgerData($parameter = [])
    {
        ...
        ## brWisePermission active for voucher entry page
        $brWisePermission = (isset($parameter['brWisePermission'])) ? $parameter['brWisePermission'] : '';
        ...
        $ledgerHeads = DB::table('acc_account_ledger as acl')
            ...
            ->where(function ($ledgerHeads) use ($selBranchArr, $branchId, $projectId, $brWisePermission) {

                ## for all branch selected project or all project
                if ($branchId != -1 && $brWisePermission != '') {
                    ...
                    if (!empty($projectId)) {
                        ...
                        $ledgerHeads->where(function ($ledgerHeads) {
                            ### branch null hoile sob asbe tai kono check dawar e dor kar nai ledger sob load hobe
                ...
                if ($branchId == -2 || $branchId > 0) {
                    ...
                    ## selected branch
                    $ledgerHeads->orWhere(function ($ledgerHeads) use ($branchId, $brWisePermission) {
                        if ($branchId == -2  && $brWisePermission != '') { ## all without HO
                            $ledgerHeads->where('acl.branch_arr', 'NOT LIKE', "1");
                        } elseif ($branchId > 0 && $brWisePermission != '') { ## selected branch

                            $ledgerHeads->where('acl.branch_arr', 'LIKE', "%,{$branchId},%")
                                ->orWhere('acl.branch_arr', 'LIKE', "{$branchId},%")
                                ->orWhere('acl.branch_arr', 'LIKE', "%,{$branchId}")
                                ->orWhere('acl.branch_arr', 'LIKE', "{$branchId}")
                                ->orWhere('acl.branch_arr', 'LIKE', "%-0%");
                            ##if ledger set for all branch then no need to check branch Id in branch_arr
                        }
                    });
                }

                ## Get data for multiple branch
                if (count($selBranchArr) > 0 && $branchId != -1 && $brWisePermission != '') {

                    foreach ($selBranchArr as $branch) {
                        $ledgerHeads->orWhere(function ($ledgerHeads) use ($branch) {
    ...
    public static function getInformationForSummaryData($parameter = [], $searchBy, $fromData, $toDate, $fiscalYearId = null)
    {
                ...
                if ($startDateOP == $endDateOP) {
                    ...
                    if ($durationMonthCount > 2) {
                        ...
                        foreach ($selBranchArr as $singleB) {
                            ...
                            if (
                                isset($branchWithAccStartDates[$singleB]) && $branchWithAccStartDates[$singleB]
        ...
        if ($beforePeriodDataFetch == true) {
            ...
            $acc_year_end =  DB::table('acc_year_end')
                ->where([['is_delete', 0]])
                ->whereIn('fiscal_year_id', $pre_fiscal_year_data->pluck('id')->toarray())
                ->select('fiscal_year_id')
                ->get();

            if (count($pre_fiscal_year_data->toArray()) > 0 && count($acc_year_end)  > 0) {
    ...
    {
        $voucherModel      = 'App\\Model\\Acc\\Voucher';
        $voucherDetailsModel      = 'App\\Model\\Acc\\VoucherDetails';

        if (DB::getSchemaBuilder()->hasTable('acc_voucher') == false
            || DB::getSchemaBuilder()->hasTable('acc_voucher_details') == false
        ) {

            $notification = array(
                'message'       => 'Unsuccessful to inserted Voucher. Table not Found',
                'alert-type'    => 'error'
            );
        ...
        $RequestData['prep_by']    = (isset($RequestData['prep_by']) && !empty($RequestData['prep_by'])) ? $RequestData['prep_by'] : Auth::id();
        $RequestData['module_id']  = (isset($RequestData['module_id']) && !empty($RequestData['module_id'])) ? $RequestData['module_id'] : Common::getModuleId();
        $RequestData['v_generate_type']  = (isset($RequestData['v_generate_type']) && !empty($RequestData['v_generate_type'])) ? $RequestData['v_generate_type'] : 0;
        $RequestData['ft_id']  = (isset($RequestData['ft_id']) && !empty($RequestData['ft_id'])) ? $RequestData['ft_id'] : 0;
        $RequestData['ft_from']  = (isset($RequestData['ft_from']) && !empty($RequestData['ft_from'])) ? $RequestData['ft_from'] : null;
        $RequestData['voucher_status']  = (isset($RequestData['voucher_status']) && !empty($RequestData['voucher_status'])) ? $RequestData['voucher_status'] : 0;
        $RequestData['reference_id']  = (isset($RequestData['reference_id']) && !empty($RequestData['reference_id'])) ? $RequestData['reference_id'] : 0;

        if (!empty($RequestData['voucher_code'])) {

            $vouchercode = DB::table("acc_voucher")->where('voucher_code', $RequestData['voucher_code'])->first();
        ...
        try {
            ...
            $insertData =  [
                ...
                'global_narration' => $RequestData['global_narration'],
                'total_amount' => round(array_sum($amount_arr)),
                'v_generate_type' => $RequestData['v_generate_type'],
                'prep_by' => $RequestData['prep_by'],
                'ft_id' => $RequestData['ft_id'],
                'ft_from' => $RequestData['ft_from'],
                'company_id' => $RequestData['company_id'],
                'voucher_status' => $RequestData['voucher_status'],
            ...
            if (class_exists($voucherModel) == true) {
                $isInsert = $voucherModel::create($insertData);
                $voucherInserted = $voucherModel::where(['voucher_code' => $RequestData['voucher_code']])->first();
            } else {
                $isInsert = DB::table("acc_voucher")->insert($insertData);
                $voucherInserted = DB::table("acc_voucher")->where(['voucher_code' => $RequestData['voucher_code']])->first();
            }
            ...
            if ($isInsert) {
                ...
                ## company type 2 fashon house
                if (Common::getCompanyType() == 2) {
                ...
                foreach ($amount_arr as $key => $Row) {
                    if (!empty($Row)) {
                        ...
                        if (class_exists($voucherDetailsModel) == true) {
                            $isInsertDetails = $voucherDetailsModel::create($RequestData2);
                        } else {
                            $isInsertDetails = DB::table("acc_voucher_details")->insert($RequestData2);
                        }
                    }
                }
    ...
    public static function updateVoucher(Request $req)
    {
        ...
        if (
            DB::getSchemaBuilder()->hasTable('acc_voucher') == false
            || DB::getSchemaBuilder()->hasTable('acc_voucher_details') == false
        ) {
        ...
        try {
            ...
            if (class_exists($voucherModel) == true) {
                $voucherInserted = $voucherModel::where(['voucher_code' => $RequestData['voucher_code']])->first();
                $isInsert = $voucherInserted->update($insertData);
            } else {
                $isInsert = DB::table('acc_voucher')->where('voucher_code', $RequestData['voucher_code'])->update($insertData);
                $voucherInserted = DB::table("acc_voucher")->where(['voucher_code' => $RequestData['voucher_code']])->first();
            }
            ...
            if ($isInsert) {
                ...
                if (class_exists($voucherDetailsModel) == true) {
                    $deleteDetailsObjs = $voucherDetailsModel::where('voucher_id', $voucherInserted->id)->get();
                } else {
                    $deleteDetailsObjs = DB::table("acc_voucher_details")->where('voucher_id', $voucherInserted->id)->get();
                }
                ...
                foreach ($amount_arr as $key => $Row) {
                    if (!empty($Row)) {
                        ...
                        if (!empty($updateid)) {
                            ...
                            if (class_exists($voucherDetailsModel) == true) {
                                $isInsertDetails = $voucherDetailsModel::where('id', $updateid->id)->update($RequestData2);
                            } else {
                                $isInsertDetails = DB::table('acc_voucher_details')->where('id', $updateid->id)->update($RequestData2);
                            }
                        } else {
                            if (class_exists($voucherDetailsModel) == true) {
                                $isInsertDetails = $voucherDetailsModel::create($RequestData2);
                ...
                if (class_exists($voucherDetailsModel) == true) {
                    ...
                    $voucherDetailsModel::whereIn('id', $tobeDeleteIDs->pluck('id')->toArray())->get()->each->delete();
                } else {
                    $tobeDeleteIDs = $deleteDetailsObjs->whereNotIn('id', $updateIDArray);
                    DB::table("acc_voucher_details")->whereIn('id', $tobeDeleteIDs->pluck('id')->toArray())->delete();
                }
    ...
    public static function insertVoucherShop(Request $req)
    {
        $RequestData = array();
        $RequestData = $req->all();
        ...
        try {
            ...
            $isInsert = DB::table("acc_voucher_shop")->insertGetId($insertData);

            // $voucherInserted = DB::table("acc_voucher_shop")->where(['voucher_code' => $RequestData['voucher_code']])->first();

            if ($isInsert) {
                /* Child Table Insertion */
                // $RequestData2['branch_id'] = $RequestData['branch_id'];
                $RequestData2 = array();
                $RequestData2['voucher_id'] = $isInsert;
                $RequestData2['ft_from']    = (!empty($RequestData['ft_from'])) ? $RequestData['ft_from'] : 0;
                ...
                foreach ($amount_arr as $key => $Row) {
                    if (!empty($Row)) {
                        ...
                        // $isInsertDetails = VoucherDetails::create($RequestData2);
                        $isInsertDetails = DB::table("acc_voucher_details_shop")->insert($RequestData2);
                    }
                }
    ...
    public static function updateVoucherShop(Request $req)
    {
        $RequestData = array();
        $RequestData = $req->all();
        ...
        if (empty($RequestData['id']) || $RequestData['id'] == null) {

            $notification = array(
                'message'    => 'ID can not be empty for update !!',
                'alert-type' => 'error',
            );
            ...
            if (count($amount_arr) <= 0) {
                $isInsert = DB::table('acc_voucher_shop')->where('id', $RequestData['id'])->update(['is_delete' => 1]);
            }
        }
        ...
        try {
            ...
            $isInsert = DB::table('acc_voucher_shop')->where('id', $RequestData['id'])->update($insertData);
            ...
            if ($isInsert) {
                ...
                /* Child Table Insertion */
                $RequestData2['voucher_id'] = $RequestData['id'];
                $RequestData2['ft_from']    = (!empty($RequestData['ft_from'])) ? $RequestData['ft_from'] : 0;
                ...
                $deleteDetailsObjs = DB::table("acc_voucher_details_shop")->where('voucher_id', $RequestData['id'])->get();
                ...
                foreach ($amount_arr as $key => $Row) {
                    if (!empty($Row)) {
                        ...
                        if (!empty($updateid)) {
                            array_push($updateIDArray, $updateid->id);
                            $isInsertDetails = DB::table('acc_voucher_details_shop')->where('id', $updateid->id)->update($RequestData2);
                        } else {
                            $isInsertDetails = DB::table("acc_voucher_details_shop")->insert($RequestData2);
                        }
                    }
                ...
                $tobeDeleteIDs = $deleteDetailsObjs->whereNotIn('id', $updateIDArray);
                DB::table("acc_voucher_details_shop")->whereIn('id', $tobeDeleteIDs->pluck('id')->toArray())->delete();
    ...
    public static function insertVoucherShopBackup(Request $req)
    {
        $RequestData = array();
        $RequestData = $req->all();
    ...
    public static function insertVouchermfn(Request $req)
    {
        ...
        if (count($amount_arr) <= 0) {
            $previousVoucher = DB::table("acc_voucher")
                ->where([
                    ['voucher_date', $RequestData['voucher_date']],
                    ['v_generate_type', $RequestData['v_generate_type']],
                    ['branch_id', $RequestData['branch_id']],
        ...
        if (empty($RequestData['voucher_code']) || $RequestData['voucher_code'] == null) {
            ...
            try {
                ...
                $isInsert        = DB::table("acc_voucher")->insert($insertData);
                $lastInsertQuery = DB::table("acc_voucher")->where('voucher_code', $RequestData['voucher_code'])->first();
                ...
                if ($isInsert) {
                    ...
                    // if (count($amount_arr) == 0) {
                    //     DB::table("acc_voucher")->where('voucher_code', $RequestData['voucher_code'])->delete();
                    // }
            ...
            try {
                ...
                if ($isInsert) {
                    ...
                    $deleteDetailsObjs = DB::table("acc_voucher_details")->where('voucher_id', $lastInsertQuery->id)->get();
    ...
    public static function generateLedgerSysCode($parent_id = null, $accType = null, $gHead = null, $company_id = null)
    {
        ...
        if ($parent_id == 0) {
            $AccTypeData = DB::table("acc_account_type")->where([['is_delete', 0], ['is_active', 1], ['id', $accType]])
                ->select('code')
                ->first();
            ...
            $parentLedger = DB::table("acc_account_ledger")->where([['is_delete', 0], ['is_active', 1], ['id', $parent_id], ['company_id', $company_id]])
                ->select('sys_code')
                ->first();
            ...
            if ($parentLedger) {
                ...
                $LedgerCount = DB::table("acc_account_ledger")->where([['is_delete', 0], ['is_active', 1], ['company_id', $company_id]])
    ...
    public static function LedgerHTML($GlobalRole = null, $AccType = null, $branchId = null, $projectId = null, $projectTypeId = null)
    {
        ...
        $ledgerHeads = self::getLedgerData(['branchId' => $branchId, 'projectId' => $projectId, 'accType' => $AccType, 'brWisePermission'=>"voucher_index"]);
        ...
        if (count($ledgerHeads) > 0) {
            ...
            foreach ($QueryData as $RootData) {
                ...
                $html .= '<tr>';
                // .$RootData->id.'-'
                $html .= "<td class='text-center'> " . self::$GlobalCount . " </td>";
                self::$GlobalCount++;
                $html .= '<td>';
    ...
    public static function SubLedgerHTML($ParentID = null, $ParentArr = [], $count = null, $GlobalRole = null, $accTypeData = [])
    {
        ...
        if (isset($ParentArr[$ParentID])) {
            ...
            foreach ($SubArrData as $Subdata) {
                ...
                $subHtml .= '<tr>';
                $subHtml .= "<td class='text-center'> " . self::$GlobalCount . "</td>";
                self::$GlobalCount++;
    ...
    public static function getAutoVoucherRefinedLedger(
        ...
        $ledger = self::getLedgerAccount($branchId, $projectId, $projectTypeId, $accType, $groupHead, $level, $brPermission);
        ...
        if (Common::getDBConnection() == "sqlite") {
            ...
            $queryData = DB::table('acc_voucher as av')
                ...
                ->select(
                    ...
                    av.branch_id, CONCAT(br.branch_name, " [", br.branch_code, "]") as branch_name'
                    )
                )
                ->orderBy('av.voucher_date', 'ASC')
                ->groupBy('av.branch_id')
        ...
        if (Common::getDBConnection() == "sqlite") {
            ...
            $obData = DB::table('acc_ob_m as obm')
                ...
                ->select(
                    ...
                    obm.branch_id, CONCAT(br.branch_name, " [", br.branch_code, "]") as branch_name'
                    )
                )
                ->orderBy('obm.opening_date', 'ASC')
                ->groupBy('obm.branch_id')
    ...
    public static function fnCashBankBalanceFromDaySummary($balanceFor, $parameter = [])
    {
        ...
        $selBranchArr = (isset($parameter['branchArr'])) ? $parameter['branchArr'] : Common::getBranchIdsForAllSection(['branchId' => $branchId, 'zoneId' => $zoneId, 'regionId' => $regionId, 'areaId' => $areaId]);
    ...
    public static function fnCashBankBalance($balanceFor, $parameter = [])
    {
        ...
        $selBranchArr = (isset($parameter['branchArr'])) ? $parameter['branchArr'] : Common::getBranchIdsForAllSection([
            'companyId'     => $companyId,
            'projectId'     => $projectId,
            'projectTypeId' => $projectTypeId,
            'branchId'      => $branchId,
            ...
            if (isset($opFetchFrom['yearEnd'])) {
                ...
                if (count($opYEMaster) > 0) {
                    ...
                    $onPeriodData = DB::table('acc_year_end_balance_d')
                        ...
                        ->groupBy('ledger_id')
                        ->selectRaw('ledger_id,
                            SUM(IFNULL(cumulative_debit_amount, 0)) as s_debit_amount, SUM(IFNULL(cumulative_credit_amount, 0)) as s_credit_amount')
    ...
    public static function fnCashBankBalance_new($balanceFor, $parameter = [])
    {
        $companyId     = (isset($parameter['companyId'])) ? $parameter['companyId'] : Common::getCompanyId();
        $projectId     = (isset($parameter['projectId'])) ? $parameter['projectId'] : Common::getProjectId();
        $projectTypeId = (isset($parameter['projectTypeId'])) ? $parameter['projectTypeId'] : Common::getProjectTypeId();
        ...
        if (isset($opFetchFrom['thisMonth'])) {

            $startDateTM = $opFetchFrom['thisMonth']['startDate'];
            $endDateTM = $opFetchFrom['thisMonth']['endDate'];

            $tmVoucherMQuery = DB::table('acc_voucher')
                ->where([['is_delete', 0], ['is_active', 1]])
                ->whereIn('voucher_status', [1, 2])
                ->whereIn('branch_id', $selBranchArr)
                ->where(function ($tmVoucherMQuery) use ($startDateTM, $endDateTM) {
                    $tmVoucherMQuery->where('voucher_date', null);

                    if (!empty($startDateTM) && !empty($endDateTM)) {
                        $tmVoucherMQuery->orWhereBetween('voucher_date', [$startDateTM->format('Y-m-d'), $endDateTM->format('Y-m-d')]);
                    }
                })
                ->where(function ($tmVoucherMQuery) use ($companyId, $projectId, $projectTypeId) {

                    if (!empty($companyId)) {
                        $tmVoucherMQuery->where('company_id', $companyId);
                    }

                    if (!empty($projectId)) {
                        $tmVoucherMQuery->where('project_id', $projectId);
                    }

                    if (!empty($projectTypeId)) {
                        $tmVoucherMQuery->where('project_type_id', $projectTypeId);
                    }
                })
                ->orderBy('voucher_date', 'ASC')
                ->get();

            $tmVoucherM = $tmVoucherMQuery->pluck('id')->toarray();

            ## this Period transaction
            if (count($tmVoucherM) > 0) {

                $thisMonthDebit = DB::table('acc_voucher_details')
                    ->whereIn('voucher_id', $tmVoucherM)
                    ->whereIn('debit_acc', $transactionLedgerArr)
                    ->groupBy('debit_acc')
                    ->selectRaw('debit_acc as ledger_id, SUM(IFNULL(amount, 0)) as debit_amount')
                    ->get(['ledger_id', 'debit_amount'])
                ...
                foreach ($thisMonthDebit as $ledger => $row) {

                    if (!isset($thisMonthData[$ledger])) {
                        $thisMonthData[$ledger]                = new \stdClass();
                        $thisMonthData[$ledger]->ledger_id     = $ledger;
                        $thisMonthData[$ledger]->s_debit_amount = $row->debit_amount;
                        $thisMonthData[$ledger]->s_credit_amount = 0;
                        $thisMonthData[$ledger]->s_cash_debit  = 0;
                        $thisMonthData[$ledger]->s_cash_credit = 0;
                        $thisMonthData[$ledger]->s_bank_debit  = 0;
                        $thisMonthData[$ledger]->s_bank_credit = 0;
                        $thisMonthData[$ledger]->s_jv_debit    = 0;
                        $thisMonthData[$ledger]->s_jv_credit   = 0;
                        $thisMonthData[$ledger]->s_ft_debit    = 0;
                        $thisMonthData[$ledger]->s_ft_credit   = 0;
                    } else {
                        $thisMonthData[$ledger]->s_debit_amount +=  $row->debit_amount;
                    }
                }
                ...
                foreach ($thisMonthCredit as $ledger => $row) {

                    if (!isset($thisMonthData[$ledger])) {
                        $thisMonthData[$ledger]                = new \stdClass();
                        $thisMonthData[$ledger]->ledger_id     = $ledger;
                        $thisMonthData[$ledger]->s_debit_amount = 0;
                        $thisMonthData[$ledger]->s_credit_amount = $row->credit_amount;
                        $thisMonthData[$ledger]->s_cash_debit  = 0;
                        $thisMonthData[$ledger]->s_cash_credit = 0;
                        $thisMonthData[$ledger]->s_bank_debit  = 0;
                        $thisMonthData[$ledger]->s_bank_credit = 0;
        ...
        foreach ($cashLedgerIds as $ledgerId) {
            ...
            if (isset($obData[$ledgerId])) {
                $thisOb = $obData[$ledgerId];
                $amount_bp_for_cash += $thisOb->s_debit_amount - $thisOb->s_credit_amount;
            }
        ...
        foreach ($bankLedgerIds as $ledgerId) {
            ...
            if (isset($obData[$ledgerId])) {
                $thisOb = $obData[$ledgerId];
                $amount_bp_for_bank += $thisOb->s_debit_amount - $thisOb->s_credit_amount;
            }
        ...
        $resultData['onPeriod'] = $cumulativeCashAndBank;
    ...
    public static function fnGetselectedBranchArr($branchId = null, $areaId = null, $zoneId = null, $regionId = null)
    {

        return Common::getBranchIdsForAllSection([
            'branchId' => $branchId,
            'zoneId' => $zoneId,
            'regionId' => $regionId,
            'areaId' => $areaId
    ...
    public static function backup_15062023_fnGetselectedBranchArr($branchId = '', $areaId = '', $zoneId = '')
    {
