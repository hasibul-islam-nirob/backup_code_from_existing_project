...
use App\Model\GNL\HR\EmployeeResign;
...
class EmployeeResignController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $totalRecords = EmployeeResign::select('count(*) as allcount')->count();
            $totalRecordswithFilter = EmployeeResign::select('count(*) as allcount')->where('resign_code', 'like', '%' . $searchValue . '%')->count();
            ...
            $allData  = EmployeeResign::from('hr_app_resigns AS apl')
                ...
                ->when(true, function ($query) use ($request, $userInfo, $searchValue, $selBranchArr) { //Searching
                    ...
                    if (!empty($searchValue)) {
                        ...
                        $query->where('resign_code', 'like', '%' . $searchValue . '%');
                    }
    ...
    public function insert(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                $appl->attachment = ($request->hasFile('attachment')) ? Common::fileUpload($request->file('attachment'), 'employee_resign', '') : null;
    ...
    public function get($id)
    {
        ...
        $queryData = EmployeeResign::with('branch', 'employee')->find($id);
    ...
    {
        $appl    = EmployeeResign::find(decrypt($id));
    ...
    {
        $targetRow            = EmployeeResign::where('id', decrypt($id))->first();
