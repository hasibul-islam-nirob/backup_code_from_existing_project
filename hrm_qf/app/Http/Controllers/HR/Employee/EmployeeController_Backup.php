...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $allData = Employee::from('hr_employees as emp')
                ...
                ->when(true, function ($query) use ($columnName, $columnSortOrder, $request) {
                ...
                ->where(function ($query) use ($request, $searchValue, $selBranchArr) {
    ...
    public function add_draft(Request $request){
        ...
        if ($request->isMethod('post')) {
            ...
            if ($passport['isValid'] == false) {
                ...
                $formTypeArr = ['General','Organization','Account','Education','Training','Experience','Guarantor','Nominee','Reference'];
                ...
                try {
                    ...
                    if($formTypeArr[0] == $request->submittedFrom){
                        ...
                        $sysUserData = [
                            ...
                            //'user_image' => ($request->hasFile('emp_photo')) ? Common::fileUpload($request->file('emp_photo'), 'system_user', '') : null,
                    ...
                    }elseif($formTypeArr[1] == $request->submittedFrom){
                        ...
                        $sysUserId = DB::table('gnl_sys_users')->where('emp_id', $request['emp_id'])->updateGetId($sysUserData);
                        /* Insert Into gnl_system_user table */
    ...
    public function insertEmpGeneralData($data){
        dd($data->all());
        ...
        if ($request->isMethod('post')) {
            ...
            if ($passport['isValid'] == false) {
                ...
                if(isset($request['submittedFrom']) && $request['submittedFrom'] == 'General'){
                    ...
                    $this->insertEmpGeneralData($request);
                ...
                try {
                    ...
                    /* $accData = [
                    'emp_id' => $id,
                    'bank_id' => $request['acc_bank_id'],
                    'bank_branch_id' => $request['acc_bank_branch_id'],
                    'bank_acc_type' => $request['acc_bank_acc_type'],
                    'bank_acc_number' => $request['acc_bank_acc_number'],
                    ];
                    ...
                    $govtGuar = [
                        ...
                        'photo'           => ($request->hasFile('govt_guar_photo')) ? Common::fileUpload($request->file('govt_guar_photo'), $tableName, $id) : null,
                        'signature'       => ($request->hasFile('govt_guar_signature')) ? Common::fileUpload($request->file('govt_guar_signature'), $tableName, $id) : null,
                        'status'          => 1,
                    ];
                    ...
                    $relGuar = [
                        ...
                        'photo'           => ($request->hasFile('rel_guar_photo')) ? Common::fileUpload($request->file('rel_guar_photo'), $tableName, $id) : null,
                        'signature'       => ($request->hasFile('rel_guar_signature')) ? Common::fileUpload($request->file('rel_guar_signature'), $tableName, $id) : null,
                        'status'          => 1,
                    ];
                    ...
                    for ($i = 0; $i < count($request['nom_name']); $i++) {
                        $nomData = [
                            ...
                            'photo'      => ($request->hasFile('nom_photo')) ? Common::fileUpload($request->file('nom_photo')[$i], $tableName, $id) : null,
                            'signature'  => ($request->hasFile('nom_signature')) ? Common::fileUpload($request->file('nom_signature')[$i], $tableName, $id) : null,
                            'status'     => 1,
                        ];
                        //Insert Here......................
    ...
    public function getPassport($request, $operationType = null)
    {
        ...
        $errorMsg = ($operationType == 'store' && isset($request['emp_code'])) ? ($this->isDuplicateEmployeeCode($request['emp_code']) ? $errorMsg . '<br/> Duplicate employee code' : $errorMsg) : $errorMsg;
    ...
    public function isDuplicateEmployeeCode($emp_code): bool
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($passport['isValid'] == false) {
                ...
                try {
                    ...
                    /* if ($request['org_username'] != null && $request['org_password'] != null){
                            SysUser::where('id', $edata->user_id)->update([
                                'username' => $request['org_username'],
                                'password' => Hash::make($request['org_password']),
                            ]);
                    }
                    else if ($request['org_username'] != null){
                        SysUser::where('id', $edata->user_id)->update([
                            'username' => $request['org_username'],
                        ]);
                    } */
                    ...
                    EmployeeAccountDetails::where('emp_id',$decId)->update($accData); */
