...
class PensionSchemeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $totalRecords = DB::table('hr_payroll_settings_pension')->select('count(*) as allcount')->where('is_delete', 0)->count();
            $totalRecordswithFilter = DB::table('hr_payroll_settings_pension')->select('count(*) as allcount')->where('is_delete', 0)->count();

            $allData  = PensionScheme::where('is_delete', 0)
    ...
    public function insert(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                $id = DB::table('hr_payroll_settings_pension')->insertGetId([
                ...
                foreach($request->benefit_y as $key => $val){
                    if($request->benefit_y[$key] != '' && $request->calculation_type[$key] != ''){
                        DB::table('hr_payroll_settings_pension_details')->insert([
                            'pension_id' => $id,
                            'benefit_y' => $request->benefit_y[$key],
    ...
    public function update(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                DB::table('hr_payroll_settings_pension')->where('id', $id)->update([
                    // 'group_id' => $request->group_id,
                    'company_id' => $request->company_id,
                    'project_id' => $request->project_id,
                    'effective_date' => (new DateTime($request->effective_date))->format('Y-m-d'),
                ...
                DB::table('hr_payroll_settings_pension_details')->where('pension_id', $id)->delete();
    ...
    {
        $delete = DB::table('hr_payroll_settings_pension')->where('id', decrypt($id))->update(['is_delete' => 1]);
        //DB::table('hr_payroll_settings_pension_details')->where('pension_id', decrypt($id))->delete();
    ...
    {
        $targetRow            = PensionScheme::where('id', decrypt($id))->first();
