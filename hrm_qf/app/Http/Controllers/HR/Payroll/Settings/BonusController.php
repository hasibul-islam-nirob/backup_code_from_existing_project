...
class BonusController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $totalRecords = DB::table('hr_payroll_settings_bonus')->select('count(*) as allcount')->where('is_delete', 0)->count();
            $totalRecordswithFilter = DB::table('hr_payroll_settings_bonus')->select('count(*) as allcount')->where('is_delete', 0)->count();

            $allData  = Bonus::where('is_delete', 0)
            ...
            foreach ($allData as $key => $row) {
                ...
                $data[$key]['religion'] = $row->religion_id == 0 ? 'All Religion' : $row->religion()->name;
                $data[$key]['calculation_percentage'] = $row->calculation_percentage;
                $data[$key]['based_on'] = $row->based_on;
                $data[$key]['min_job_m'] = $row->min_job_m;
    ...
    public function insert(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                $id = DB::table('hr_payroll_settings_bonus')->insertGetId([
                    'name' => $request->name,
                    ...
                    'calculation_percentage' => $request->calculation_percentage,
                    'based_on' => $request->based_on,
                    'min_job_m' => $request->min_job_m,
    ...
    public function update(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                DB::table('hr_payroll_settings_bonus')->where('id', $id)->update([
                    'name' => $request->name,
                    // 'group_id' => $request->group_id,
                    'company_id' => $request->company_id,
                    'project_id' => $request->project_id,
    ...
    {
        $delete = DB::table('hr_payroll_settings_bonus')->where('id', decrypt($id))->update(['is_delete' => 1]);
        //DB::table('hr_payroll_settings_bonus_details')->where('bonus_id', decrypt($id))->delete();
    ...
    {
        $targetRow            = Bonus::where('id', decrypt($id))->first();
