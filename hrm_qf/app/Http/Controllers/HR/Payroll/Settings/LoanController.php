...
class LoanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $totalRecords = DB::table('hr_payroll_settings_loan')->select('count(*) as allcount')->where('is_delete', 0)->count();
            $totalRecordswithFilter = DB::table('hr_payroll_settings_loan')->select('count(*) as allcount')->where('is_delete', 0)->count();

            $allData  = Loan::where('is_delete', 0)->skip($start)->take($rowperpage)->get();
            ...
            foreach ($allData as $key => $row) {
                ...
                $data[$key]['id']          = $sno;
                $data[$key]['vehicle_type']        = $row->vehicle_type;
                $data[$key]['max_installment']        = $row->max_installment;
                $data[$key]['max_amount']        = $row->max_amount;
                $data[$key]['settlement_fee']        = $row->settlement_fee;
                
                $data[$key]['effective_date'] = (new DateTime($row->effective_date))->format('d-m-Y');

                $data[$key]['intrest_rate']        = $row->intrest_rate;
                $data[$key]['intrest_method']        = ($row->intrest_method == 'decline') ? 'Decline' : 'Flate ';
    ...
    public function insert(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                $id = DB::table('hr_payroll_settings_loan')->insertGetId([
                    ...
                    'intrest_method' => $request->intrest_method,
    ...
    public function update(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                DB::table('hr_payroll_settings_loan')->where('id', $id)->update([
                    'vehicle_type' => $request->vehicle_type,
                    'max_installment' => $request->max_installment,
                    'max_amount' => $request->max_amount,
    ...
    {
        $delete = DB::table('hr_payroll_settings_loan')->where('id', decrypt($id))->update(['is_delete' => 1]);
        //DB::table('hr_payroll_settings_loan_details')->where('loan_id', decrypt($id))->delete();
    ...
    {
        $targetRow            = Loan::where('id', decrypt($id))->first();
