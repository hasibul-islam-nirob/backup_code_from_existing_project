...
class GratuityController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $totalRecords = DB::table('hr_payroll_settings_gratuity')->select('count(*) as allcount')->where('is_delete', 0)->count();
            $totalRecordswithFilter = DB::table('hr_payroll_settings_gratuity')->select('count(*) as allcount')->where('is_delete', 0)->count();

            $allData  = Gratuity::where('is_delete', 0)
    ...
    public function insert(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                $id = DB::table('hr_payroll_settings_gratuity')->insertGetId([
                ...
                foreach($request->steps as $key => $val){
                    if($request->steps[$key] != '' && $request->year_from[$key] != '' && $request->year_to[$key] != '' && $request->gratuity[$key] != ''){
                        DB::table('hr_payroll_settings_gratuity_details')->insert([
                            'gratuity_id' => $id,
                            'steps' => $request->steps[$key],
                            'year_from' => $request->year_from[$key],
                            'year_to' => $request->year_to[$key],
    ...
    public function update(Request $request)
    {
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                DB::table('hr_payroll_settings_gratuity')->where('id', $id)->update([
                    // 'group_id' => $request->group_id,
                    'company_id' => $request->company_id,
                    'project_id' => $request->project_id,
                    'effective_date' => (new DateTime($request->effective_date))->format('Y-m-d'),
                ...
                DB::table('hr_payroll_settings_gratuity_details')->where('gratuity_id', $id)->delete();
    ...
    {
        $delete = DB::table('hr_payroll_settings_gratuity')->where('id', decrypt($id))->update(['is_delete' => 1]);
        //DB::table('hr_payroll_settings_gratuity_details')->where('gratuity_id', decrypt($id))->delete();
    ...
    {
        $targetRow            = Gratuity::where('id', decrypt($id))->first();
