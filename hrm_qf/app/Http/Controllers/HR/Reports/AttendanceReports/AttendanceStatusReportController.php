...
class AttendanceStatusReportController extends Controller
{
    ...
    public function loadstatus(Request $request)
    {
        ...
        foreach ($employeeData as $key => $row) {
            ...
            foreach ($empAttendanceData as $date => $rowData) {
                $rowFirst = $rowData[0];
                $on_Duty = $off_Duty = $onDuty = $offDuty = '-';
                ...
                foreach ($monthDates as $keyDate => $value) {
                    ...
                    if(in_array($keyDate, $holidays)){
                        ...
                        $employeeData[$key]->attendance[$keyDate]['early_out'] = '-';
                        $employeeData[$key]->attendance[$keyDate]['work_time'] = '-';
                        $employeeData[$key]->attendance[$keyDate]['status'] = 'Holiday';
                    }
                }
        ...
        return view('HR.Reports.AttendanceReports.StatusReport.report_body', compact('employeeData', 'orderStatus', 'empAttendanceData','withHoliday', 'withAbsent', 'holidays','attendance_bypass_arr'));
    }
}
