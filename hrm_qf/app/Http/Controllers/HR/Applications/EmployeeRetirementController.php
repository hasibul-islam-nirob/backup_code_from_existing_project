...
class EmployeeRetirementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $totalRecords = EmployeeRetirement::select('count(*) as allcount')->where('is_delete', 0)->count();
            $totalRecordswithFilter = EmployeeRetirement::select('count(*) as allcount')->where('is_delete', 0)->where('retirement_code', 'like', '%' . $searchValue . '%')->count();
            ...
            $allData  = EmployeeRetirement::from('hr_app_retirements AS apl')
                ...
                ->when(true, function ($query) use ($columnName, $columnSortOrder, $request) { //Ordering
                    ...
                    } elseif ($columnName == "retirement_date") {
                        $query->orderBy("apl." . $columnName, $columnSortOrder);
                ...
                ->when(true, function ($query) use ($request, $userInfo, $searchValue, $selBranchArr) {
                    ...
                    if (!empty($request->appl_code)) {
                        $query->where('apl.retirement_code', 'LIKE', "%{$request->appl_code}%");
                    }
                    if ($request->appl_status == "0" || !empty($request->appl_status)) {
                        $query->where('apl.is_active', $request->appl_status);
                    }
                    if (!empty($request->start_date) && !empty($request->end_date)) {
                        $query->whereBetween('apl.retirement_date', [(new DateTime($request->start_date))->format('Y-m-d'), (new DateTime($request->end_date))->format('Y-m-d')]);
                    } elseif (!empty($request->start_date)) {
                        $query->where('apl.retirement_date', '>=', (new DateTime($request->start_date))->format('Y-m-d'));
                    } elseif (!empty($request->end_date)) {
                        $query->where('apl.retirement_date', '<=', (new DateTime($request->end_date))->format('Y-m-d'));
                    }
                ...
                ->where(function($query2) use ($searchValue){
                    if (!empty($searchValue)) {
                        $query2->where('retirement_code', 'like', '%' . $searchValue . '%');
                    }
                })
    ...
    public function insert(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl                     = new EmployeeRetirement();
                $appl->branch_id          = $request['branch_id'];
                $appl->retirement_code        = HrService::generateRetirementCode($request['branch_id']);
                $appl->emp_id             = $request['employee_id'];
                $appl->reason             = $request['reason'];
                $appl->description        = $request['description'];
                $appl->retirement_date        = (new DateTime($request['retirement_date']))->format('Y-m-d');
                ...
                if ($status == 'send') {
                    ...
                    $first_approval = CommonController::get_first_approval(13, $permission_for, $applicant);

                    if (empty($first_approval)) {
                        (new ApplicationProcessController)->approve($appl, 13);
    ...
    public function uploadFiles($attachments, $fk = null)
    {
        ...
        foreach ($attachments as $key => $file) {
            ...
            DB::table('hr_attachments')->insert([
                ...
                'foreign_key' => (!empty($fk)) ? $fk : EmployeeRetirement::latest()->first()->id,
                'ref_table_name' => 'hr_app_retirements',
    ...
    public function update(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl = EmployeeRetirement::find(decrypt($request['retirement_id']));
                ...
                if (isset($request->fileIds) && count($request->fileIds) > 0) {
                    DB::table('hr_attachments')->where('foreign_key', decrypt($request['retirement_id']))->where('ref_table_name', 'hr_app_retirements')
    ...
    {
        return CommonController::get_application('\\App\\Model\\HR\\EmployeeRetirement', $id, ['employee', 'branch', 'attachments']);
    }

    public function send($id)
    {
        return CommonController::send_application('\\App\\Model\\HR\\EmployeeRetirement', $id, 13);
    }

    public function delete($id)
    {
        return CommonController::delete_application('\\App\\Model\\HR\\EmployeeRetirement', $id);
    }
