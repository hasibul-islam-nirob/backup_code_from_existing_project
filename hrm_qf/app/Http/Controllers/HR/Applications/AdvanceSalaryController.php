...
use App\Model\HR\AppAdvanceSalary;
...
class AdvanceSalaryController extends Controller
{
    public function getPassport($requestData, $operationType, $Data = null){
        ...
        if ($operationType == 'store' || $operationType == 'update') {
            ...
            $attributes = array(
                ...
                'first_repay_month'     => 'First Repay Month',
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $allData  = AppAdvanceSalary::from('hr_app_advance_salary as ads')
                ->where('ads.is_delete', 0)
                ->whereIn('ads.branch_id', $selBranchArr)
                ...
                */
                ->when(true, function ($query) use ($columnName, $columnSortOrder, $request) { //Ordering
                    ...
                    if ($columnName == "advance_salary_code") {
                        $query->orderBy("ads." . $columnName, $columnSortOrder);
                        ...
                        $query->orderBy('ads.is_active', $columnSortOrder);
                ...
                ->when(true, function ($query) use ($request, $userInfo, $searchValue, $selBranchArr) {
                    ...
                    if (!empty($request->employee_id)) {
                        $query->where('ads.emp_id', $request->employee_id);
                    }

                    if (!empty($request->appl_code)) {

                        $query->where('ads.advance_salary_code', 'LIKE', "%{$request->appl_code}%");
                    }

                    if ($request->appl_status == "0" || !empty($request->appl_status)) {

                        $query->where('ads.is_active', $request->appl_status);
                    }

                    if (!empty($request->start_date) && !empty($request->end_date)) {

                        $query->whereBetween('ads.application_date', [(new DateTime($request->start_date))->format('Y-m-d'), (new DateTime($request->end_date))->format('Y-m-d')]);
                    } elseif (!empty($request->start_date)) {

                        $query->where('ads.application_date', '>=', (new DateTime($request->start_date))->format('Y-m-d'));
                    } elseif (!empty($request->end_date)) {

                        $query->where('ads.application_date', '<=', (new DateTime($request->end_date))->format('Y-m-d'));
                    }

                })
                ->where(function($query2) use ($searchValue){
                    if (!empty($searchValue)) {
                        $query2->where('advance_salary_code', 'like', '%' . $searchValue . '%');
                    }
                });

            $totalRecordswithFilter = AppAdvanceSalary::where([['is_delete', 0], ['is_active', 1]])->count();
            ...
            $allData = $allData->skip($start)->take($rowperpage)->select('ads.*')->get();
            $data      = array();
            $sno = $start + 1;
            ...
            foreach ($allData as $key => $row) {
                ...
                $data[$key]['advance_salary_code']        = $row->advance_salary_code;
                $data[$key]['effective_date']        = (new DateTime($row->effective_date))->format('d-m-Y');
                $data[$key]['advanced_amount']        = $row->advanced_amount;
    ...
    public function insert(Request $request, $status){
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl                       = new AppAdvanceSalary();
                $appl->branch_id            = $request['branch_id'];
                $appl->advance_salary_code  = HRS::generateAdvanceSalaryCode($request['branch_id']);
                $appl->emp_id               = $request['employee_id'];

                $appl->project_id           = $request['project_id'];
                $appl->advanced_amount      = $request['advanced_amount'];
                $appl->installment_amount   = $request['installment_amount'];
                $appl->no_of_installment    = $request['no_of_installment'];
                $appl->first_repay_month    = $request['first_repay_month'];
                ...
                if ($status == 'send') {
                    ...
                    $first_approval = CommonController::get_first_approval(15, $permission_for, $applicant);

                    if (empty($first_approval)) {
                        (new ApplicationProcessController)->approve($appl, 15);
    ...
    public function update(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl                       = AppAdvanceSalary::find(decrypt($request['advance_salary_id']));
                $appl->branch_id            = $request['branch_id'];
                $appl->advance_salary_code  = HRS::generateAdvanceSalaryCode($request['branch_id']);
                $appl->emp_id               = $request['employee_id'];
    ...
    public function uploadFiles($attachments, $fk = null)
    {
        ...
        foreach ($attachments as $key => $file) {
            ...
            DB::table('hr_attachments')->insert([
                ...
                'foreign_key' => (!empty($fk)) ? $fk : AppAdvanceSalary::latest()->first()->id,
    ...
    {
        return CommonController::get_application('\\App\\Model\\HR\\AppAdvanceSalary', $id, ['branch', 'employee', 'attachments']);
    }

    public function send($id)
    {
        return CommonController::send_application('\\App\\Model\\HR\\AppAdvanceSalary', $id, 15);
    }

    public function delete($id)
    {
        return CommonController::delete_application('\\App\\Model\\HR\\AppAdvanceSalary', $id);
    }
