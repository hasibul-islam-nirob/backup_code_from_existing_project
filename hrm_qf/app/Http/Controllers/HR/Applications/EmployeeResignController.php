...
class EmployeeResignController extends Controller
{
    ...
    public function getPassport($requestData, $operationType)
    {
        ...
        if ($operationType == 'send') {
            ...
            $checkTx = ACCS::checkTransactionForEmployee($requestData->employee_id, 'resigning');
            if (empty($checkTx)) {
                $checkTx = BILLS::checkTransactionForEmployee($requestData->employee_id, 'resigning');
            }
            if (empty($checkTx)) {
                $checkTx = FAMS::checkTransactionForEmployee($requestData->employee_id, 'resigning');
            }
            if (empty($checkTx)) {
                $checkTx = INVS::checkTransactionForEmployee($requestData->employee_id, 'resigning');
            }
            if (empty($checkTx)) {
                $checkTx = MFNS::checkTransactionForEmployee($requestData->employee_id, 'resigning');
            }
            if (empty($checkTx)) {
                $checkTx = POSS::checkTransactionForEmployee($requestData->employee_id, 'resigning');
            }
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            // $totalRecords = EmployeeResign::select('count(*) as allcount')->where('is_delete', 0)->count();
            // $totalRecordswithFilter = EmployeeResign::select('count(*) as allcount')->where('is_delete', 0)->where('resign_code', 'like', '%' . $searchValue . '%')->count();
            ...
            $allData  = EmployeeResign::from('hr_app_resigns AS apl')
                ...
                ->when(true, function ($query) use ($request, $userInfo, $searchValue, $selBranchArr) { //Searching
                    ...
                    if (!empty($request->appl_code)) {
                        $query->where('apl.resign_code', 'LIKE', "%{$request->appl_code}%");
                    }
                    if ($request->appl_status == "0" || !empty($request->appl_status)) {
                        $query->where('apl.is_active', $request->appl_status);
                    }
                    if (!empty($request->start_date) && !empty($request->end_date)) {
                        $query->whereBetween('apl.resign_date', [(new DateTime($request->start_date))->format('Y-m-d'), (new DateTime($request->end_date))->format('Y-m-d')]);
                    } elseif (!empty($request->start_date)) {
                        $query->where('apl.resign_date', '>=', (new DateTime($request->start_date))->format('Y-m-d'));
                    } elseif (!empty($request->end_date)) {
                        $query->where('apl.resign_date', '<=', (new DateTime($request->end_date))->format('Y-m-d'));
                    }
                ...
                ->where(function($query2) use ($searchValue){
                    if (!empty($searchValue)) {
                        $query2->where('resign_code', 'like', '%' . $searchValue . '%');
                    }
                });
    ...
    public function insert(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl                     = new EmployeeResign();
                $appl->branch_id          = $request['branch_id'];
                $appl->resign_code        = HrService::generateResignCode($request['branch_id']);
                $appl->emp_id             = $request['employee_id'];
                $appl->reason             = $request['reason'];
                $appl->description        = $request['description'];
                $appl->resign_date        = (new DateTime($request['resign_date']))->format('Y-m-d');
                ...
                if ($status == 'send') {
                    ...
                    $first_approval = CommonController::get_first_approval(7, $permission_for, $applicant);

                    if (empty($first_approval)) {

                        (new ApplicationProcessController)->approve($appl, 7);
    ...
    public function uploadFiles($attachments, $fk = null)
    {
        ...
        foreach ($attachments as $key => $file) {
            ...
            DB::table('hr_attachments')->insert([
                'path' => Common::fileUpload($file, 'employee_resign', ''),
                'foreign_key' => (!empty($fk)) ? $fk : EmployeeResign::latest()->first()->id,
                'ref_table_name' => 'hr_app_resigns',
    ...
    public function update(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl = EmployeeResign::find(decrypt($request['resign_id']));
                ...
                if (isset($request->fileIds) && count($request->fileIds) > 0) {
                    DB::table('hr_attachments')->where('foreign_key', decrypt($request['resign_id']))->where('ref_table_name', 'hr_app_resigns')
    ...
    {
        return CommonController::get_application('\\App\\Model\\HR\\EmployeeResign', $id, ['branch', 'employee', 'attachments']);
    }

    public function send($id)
    {
        $apl = EmployeeResign::find(decrypt($id));
        ...
        return CommonController::send_application('\\App\\Model\\HR\\EmployeeResign', $id, 7);
    }

    public function delete($id)
    {
        return CommonController::delete_application('\\App\\Model\\HR\\EmployeeResign', $id);
    }
    ...
    public function finish($appl, $exe_date)
    {
        ...
        DB::beginTransaction();
        try {
            //1) Employee Table [Status] Change. status = 2 = Resigned
            DB::table('hr_employees')->where('id', $appl->emp_id)->update(['status' => 2, 'is_active' => 0]);
        ...
        /* DB::table('hr_approval_queries')->insert([
            [
                'query' => "update `hr_employees` set `status` = 2 where `id` = ". $appl->emp_id,
                'appl_name' => 'resign',
