...
use App\Model\HR\EmployeeIncrement;
...
class EmployeeIncrementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $totalRecords = EmployeeIncrement::select('count(*) as allcount')->where('is_delete', 0)->count();
            $totalRecordswithFilter = EmployeeIncrement::select('count(*) as allcount')->where('is_delete', 0)->where('increment_code', 'like', '%' . $searchValue . '%')->count();
            ...
            $allData  = EmployeeIncrement::from('hr_app_increment AS apl')
                ...
                */
                ->when(true, function ($query) use ($columnName, $columnSortOrder, $request) { //Ordering
                    ...
                    elseif ($columnName == "current_salary") {
                        $query->orderBy("apl." . $columnName, $columnSortOrder);
                    }
                    elseif ($columnName == "requested_salary") {
                        $query->orderBy("apl." . $columnName, $columnSortOrder);
                    }


                    elseif ($columnName == "increment_date") {
                        $query->orderBy("apl." . $columnName, $columnSortOrder);
                ...
                ->when(true, function ($query) use ($request, $userInfo, $searchValue, $selBranchArr) {
                    ...
                    if (!empty($request->appl_code)) {
                        $query->where('apl.increment_code', 'LIKE', "%{$request->appl_code}%");
                    }

                    if (!empty($request->current_salary)) {
                        $query->where('apl.current_salary', 'LIKE', "%{$request->current_salary}%");
                    }
                    if (!empty($request->requested_salary)) {
                        $query->where('apl.requested_salary', 'LIKE', "%{$request->requested_salary}%");
                    }
                    ...
                    if (!empty($request->start_date) && !empty($request->end_date)) {
                        $query->whereBetween('apl.increment_date', [(new DateTime($request->start_date))->format('Y-m-d'), (new DateTime($request->end_date))->format('Y-m-d')]);
                    } elseif (!empty($request->start_date)) {
                        $query->where('apl.increment_date', '>=', (new DateTime($request->start_date))->format('Y-m-d'));
                    } elseif (!empty($request->end_date)) {
                        $query->where('apl.increment_date', '<=', (new DateTime($request->end_date))->format('Y-m-d'));
                    }
                ...
                ->where(function($query2) use ($searchValue){
                    if (!empty($searchValue)) {
                        $query2->where('increment_code', 'like', '%' . $searchValue . '%');
                    }
                })
            ...
            foreach ($allData as $key => $row) {
                ...
                $data[$key]['reason']             = !empty($row->reasons['reason']) ? $row->reasons['reason'] : '-';

                $data[$key]['current_salary']     = $row->current_salary;
                $data[$key]['requested_salary']   = $row->requested_salary;

                $data[$key]['increment_date']      = (new DateTime($row->increment_date))->format('d-m-Y');
                ...
                $data[$key]['increment_code']        = $row->increment_code;
    ...
    public function insert(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl                     = new EmployeeIncrement();
                $appl->branch_id          = $request['branch_id'];
                $appl->increment_code        = HRS::generateIncrementCode($request['branch_id']);
                $appl->emp_id             = $request['employee_id'];

                $appl->current_salary     = $request['current_salary'];
                $appl->requested_salary   = $request['requested_salary'];

                $appl->reason             = $request['reason'];
                $appl->description        = $request['description'];
                $appl->increment_date        = (new DateTime($request['increment_date']))->format('Y-m-d');
                ...
                if ($status == 'send') {
                    ...
                    $first_approval = CommonController::get_first_approval(1, $permission_for, $applicant);

                    if (empty($first_approval)) {
                        (new ApplicationProcessController)->approve($appl, 1);
    ...
    public function uploadFiles($attachments, $fk = null)
    {
        ...
        foreach ($attachments as $key => $file) {
            ...
            DB::table('hr_attachments')->insert([
                'path' => Common::fileUpload($file, 'employee_retirement', ''),
                'foreign_key' => (!empty($fk)) ? $fk : EmployeeIncrement::latest()->first()->id,
                'ref_table_name' => 'hr_app_increment',
    ...
    public function update(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl = EmployeeIncrement::find(decrypt($request['increment_id']));
                ...
                $appl->updated_at     = (new DateTime())->format('Y-m-d');

                $appl->is_active          = ($status == 'send') ? 3 : 0;

                if (isset($request->fileIds) && count($request->fileIds) > 0) {
                    DB::table('hr_attachments')->where('foreign_key', decrypt($request['increment_id']))->where('ref_table_name', 'hr_app_increment')
    ...
    {
        return CommonController::get_application('\\App\\Model\\HR\\EmployeeIncrement', $id, ['employee', 'branch', 'attachments']);
    }

    public function send($id)
    {
        return CommonController::send_application('\\App\\Model\\HR\\EmployeeIncrement', $id, 13);
    }

    public function delete($id)
    {
        return CommonController::delete_application('\\App\\Model\\HR\\EmployeeIncrement', $id);
    }
    ...
    public function finish($appl, $exe_date)
    {
        DB::beginTransaction();
        try {
            //1) Employee Table [Status] Change. status = 5 = Retirement
            DB::table('hr_employees')->where('id', $appl->emp_id)->update(['status' => 5, 'is_active' => 0]);
        ...
        /* DB::table('hr_approval_queries')->insert([
            [
                'query' => "update `hr_employees` set `status` = 5 where `id` = ". $appl->emp_id,
                'appl_name' => 'retirement',
