...
class PfLoanController extends Controller
{
    public function getPassport($requestData, $operationType, $Data = null){
        ...
        if ($operationType == 'store' || $operationType == 'update') {
            ...
            $attributes = array(
                ...
                'emp_pf_amount'     => 'Employee PF Amount',
                'eligible_amount'     => 'Eligible Amount',
                'interest_amount'     => 'Interest Amount',
                'requested_loan_amount'     => 'Requested Loan Amount',
                'requested_no_of_loan_installment'     => 'Requested No of Loan Installment',
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $allData  = HrApplicationLoan::from('hr_app_loan as hrLoan')
                ->where([['hrLoan.is_delete', 0],['loan_type',2]])
            ...
            $totalRecordswithFilter = HrApplicationLoan::where([['is_delete', 0], ['is_active', 1],['loan_type',2]])->count();
            ...
            foreach ($allData as $key => $row) {
                ...
                $data[$key]['loan_amount']       = $row->emp_pf_amount;
                $data[$key]['installment_amount']  = $row->interest_amount;
    ...
    public function insert(Request $request, $status){
        ...
        if ($passport['isValid']) {
            ...
            try {
                ...
                $appl->loan_code            = HRS::generatePfLoanCode($request['branch_id']);
                $appl->emp_id               = $request['employee_id'];
                $appl->project_id           = $request['project_id'];
                $appl->loan_type            = 2;

                $appl->employment_age                   = $request['employment_age'];
                $appl->org_contribution_amount          = $request['org_contribution_amount'];
                $appl->emp_pf_amount                    = $request['emp_pf_amount'];
                $appl->eligible_amount                  = $request['eligible_amount'];
                $appl->interest_amount                  = $request['interest_amount'];
    ...
    public function update(Request $request, $status)
    {
        ...
        if ($passport['isValid']) {
            DB::beginTransaction();
            try {
                $appl                       = HrApplicationLoan::find(decrypt($request['loan_id']));
                $appl->branch_id            = $request['branch_id'];
                // $appl->loan_code            = HRS::generatePfLoanCode($request['branch_id']);
                $appl->emp_id               = $request['employee_id'];
                $appl->project_id           = $request['project_id'];
